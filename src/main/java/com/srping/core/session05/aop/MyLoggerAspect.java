package com.srping.core.session05.aop;
...
@Aspect // 切面
@Order(1) // 數字越小越先執行(若不設定預設就是 int 的最大值)
	   	  // 使用時機: 有多個切面程式在同一個橫切關注點要執行時
public class MyLoggerAspect {
	
	// 建立一個切入點的方法
	@Pointcut(value = "execution(* com.srping.core.session05.aop.MathCalcImpl.*(..))")
	public void pt() {}
	@Pointcut(value = "execution(* com.srping.core.session05.aop.MathCalcImpl.div(..))")
	public void pt2() {}
	
	// 前置通知 Advice: 執行該連接點之前所要執行的程式
	//@Before(value = "execution(public Integer com.srping.core.session05.aop.MathCalcImpl.add(Integer, Integer))") // 切入點表達式 Spring EL : execution(..)
	//@Before(value = "execution(* com.srping.core.session05.aop.MathCalcImpl.*(..))") // *(..) -> * 表示任意方法 , (..) 表示 0~多任意參數
	//@Before(value = "execution(public * com.srping.core.session05.aop.MathCalcImpl.*(..))") // *(..) -> * 表示任意方法 , (..) 表示 0~多任意參數
	//@Before(value = "execution(* com.srping.core.session05.aop.*.*(..))") // 在指定套件下之任意類 + 任意方法
	...
	// 後置通知 Advice: 執行在連接點呼叫完之後 , 在 SpringAOP 機制上會被配置在 finally 區塊中
