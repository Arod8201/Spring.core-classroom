...
public class BookServiceImpl implements BookService {
	...
	@Transactional(propagation = Propagation.REQUIRES_NEW,
				   ...
				   //rollbackFor = {Exceptionx.class}) // 也可以
	/*
	 * Propagation.REQUIRED (預設): 如果有事務在運行 , 當前方法就在該事務中運行
	 ...
	 * Propagation.NOTSUPPORTED  : 當前方法不應該讚事務中運行 , 如果有事務就會將它掛起不用
	 * Propagation.MANDATORY	 : 當前方法必須在運行事務中 , 若沒有就拋出異常
	 * Propagation.NEVER	 	 : 當前方法不應該運行在事務中 , 若在事務中就拋出異常
	 * Propagation.NESTED 	  	 : 如果有事務運行 , 當前方法必須嵌套在事務中 , 否則就啟動一個新事務並在自己的事務中運行
	 * 
	 * ============================================================================================
	 * 
	 * Isolation 事務的隔離級別 , 在併發的情況下 , 操作數據 CRUD 的一種規則
	 * Isolation.DEFAULT: 預設 , MySQL 預設是(Isolation.REPEATABLE_READ 可重複讀)
	 * Isolation.READ_UNCOMMITTED 讀未提交: 髒讀 (針對欄位資料)
	 * Isolation.READ_COMMITTED 讀已提交  : 不可重複讀 (針對欄位資料)
	 * Isolation.REPEATABLE_READ 可重複讀 : 幻讀 , 其他人不可以針對指定資料列 RUD (針對欄位列)
	 * Isolation.SERIALIZABLE 序列化: 效能低 , 消耗大 , 但是可以簡單解決上面的問題 (實務上不建議使用) (針對資料表)
	 * 
	 * ============================================================================================
	 * 
	 * 拋出 RuntimeException 會自動 rollback
	 * 拋出 Exception 則必須自訂 rollbackFor = Exception.class
	 * */
	@Override
